...
def menu_admin():
    ...
    if admin==1:
        ...
        ML6=Label(frameadmin2, text="Consulta de Gaveta-->",fg="#f2f5f7",font=("Verdana", 14),bg="#301fc9",width=28,anchor=E)
...
def accion_admin(tipo):
    ...
    if tipo==1:
        ...
        ML1=Label(frameaadmin0, text="Consulta de Gaveta",fg="#FF0000",font=("Verdana", 22),bg="#301fc9")
        ML1.pack(in_=frameaadmin0,fill=BOTH,expand=True)
        L1=Label(frameaadmin1,text="Dinero en Gaveta: ")
        L1.pack(in_=frameaadmin1,side=LEFT,fill=BOTH,expand=True)
        MAAE1=Entry(frameaadmin1)
        MAAE1.pack(in_=frameaadmin1,side=RIGHT,fill=BOTH,expand=True)
        MAAE1.config(state="readonly")
        L2=Label(frameaadmin2,text="Fecha Actual: ")
        L2.pack(in_=frameaadmin2,side=LEFT,fill=BOTH,expand=True)
        MAAE2=Entry(frameaadmin2)
        MAAE2.pack(in_=frameaadmin2,side=RIGHT,fill=BOTH,expand=True)
        MAAE2.config(state="readonly")
        L3=Label(frameaadmin3,text="Hora Actual: ")
        L3.pack(in_=frameaadmin3,side=LEFT,fill=BOTH,expand=True)
        MAAE3=Entry(frameaadmin3)
        MAAE3.pack(in_=frameaadmin3,side=RIGHT,fill=BOTH,expand=True)
        MAAE3.config(state="readonly")
...
def ejecutar_admin(tipo):
    ...
    if tipo==1:
        ...
        MAAE1.delete(0,END)
        MAAE1.insert(0,gaveta)
        MAAE1.config(state="readonly")
        #instanciamos datetime para que se active el builder de la clase
        ahora = datetime.now()
        #creamos mulitples instancias hijo para los argumentos requeridos
        ...
        print (resultadolistacuentas)
        print (busqueda)
        consultalista=revisar(resultadolistacuentas,busqueda)
        consultaraw="".join(consultalista)
        consulta=consultaraw.strip("''[]")
        if consulta in resultadolistacuentas:
            numeroindex=resultadolistacuentas.index(consulta)
            index2=numeroindex+1
            MAAE1.delete(0,END)
            MAAE1.insert(0,"Ej: Tulio")
            MAAE2.config(state="normal")
            MAAE2.delete(0,END)
            MAAE2.insert(0,consulta)
            MAAE2.config(state="readonly")
            MAAE3.config(state="normal")
            MAAE3.delete(0,END)
            MAAE3.insert(0,resultadolistacuentas[index2])
            MAAE3.config(state="readonly")
        else:
            MAAE1.delete(0,END)
            MAAE1.insert(0,"Ej: Tulio")
